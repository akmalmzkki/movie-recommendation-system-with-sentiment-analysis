...
from architecture.model import recommends, get_suggestions, model, convert_list_of_str_to_list, convert_str_to_list
...
def recommend():
    ...
    movie_cards = {poster: movie 
                   for poster, movie 
                   in zip(rec_posters, rec_movies)
                   }
    
    casts = {name: [cast_id, cast_char, cast_profile] 
             for name, cast_id, cast_char, cast_profile 
             in zip(cast_names, cast_ids, cast_chars, cast_profiles)
             }
    
    cast_details = {name: [cast_id, cast_profile, cast_bday, cast_place, cast_bio] 
    ...
    for reviews in soup_result:
        if reviews.string:
            ...
            percentase_reviews.append(clf.predict_proba(vectorizer.transform(np.array([reviews.string])))[0][1])

    # Menggabungkan ulasan dan komentar menjadi ke dalam dictionary
    movie_reviews = {reviews_list[i]: round(percentase_reviews[i], 3) for i in range(len(reviews_list))}     
